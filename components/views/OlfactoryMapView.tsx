...
interface OlfactoryMapViewProps {
    molecules: Molecule[];
...
const OlfactoryMapView: React.FC<OlfactoryMapViewProps> = ({ molecules, onSelectMolecule, currencySymbol }) => {
    ...
    const SortIcon: React.FC<{ field: SortField }> = ({ field }) => {
        if (sortField !== field) return <span className="ml-1 text-gray-600 opacity-0 group-hover:opacity-50">↕</span>;
    ...
    return (
        <div className="animate-fade-in pt-12 h-full flex flex-col space-y-6">
            <div className="flex-shrink-0">
                <h1 className="text-3xl font-bold text-white mb-2">Scent Bank</h1>
                <p className="text-gray-400">A physics-based view of every molecule in your library, color-coded by family.</p>
            </div>
            
            {/* Map Container */}
            <div className="h-[500px] relative bg-[#111] border border-gray-800 rounded-lg overflow-hidden flex-shrink-0 p-4">
                <OlfactoryMindmap 
            ...
            <div className="flex-grow bg-[#1C1C1C] border border-gray-800 rounded-lg p-4 overflow-hidden flex flex-col min-h-[400px]">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 space-y-3 md:space-y-0">
                    ...
                    <div className="relative w-full md:w-64">
                        <input 
                            ...
                            className="w-full bg-[#111] border border-gray-700 rounded-md px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#a89984] pl-9"
                        />
                ...
                <div className="overflow-y-auto flex-grow">
                    <table className="w-full text-sm text-left text-gray-400">
                        <thead className="text-xs text-gray-400 uppercase bg-[#111] sticky top-0">
                            <tr>
                                <th 
                                    className="px-4 py-3 cursor-pointer hover:text-white group select-none"
                                ...
                                <th 
                                    className="px-4 py-3 text-right cursor-pointer hover:text-white group select-none"
                        ...
                        <tbody className="divide-y divide-gray-800">
                            {purchaseHistory.map(item => (
                                <tr key={item.id} className="hover:bg-gray-800 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-right font-mono text-gray-300">{item.amount}</td>
...
export default OlfactoryMapView;
